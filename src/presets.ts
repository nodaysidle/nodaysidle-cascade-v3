...
const swiftPermissionPatterns: Readonly<Record<PermissionCapability, string>> = {
  ...
  network: "Use URLSession for requests to the declared services. App Transport Security allows only HTTPS: a declared http endpoint needs an NSAppTransportSecurity exception in Info.plist, such as NSAllowsLocalNetworking for a localhost or local-network server. The unsandboxed local build needs no network entitlement.",
  ...
  "global-input": "Register shortcuts with Carbon RegisterEventHotKey; use an event tap only when required and explain Input Monitoring denial. RegisterEventHotKey cannot bind a modifier key on its own (such as Right Option): for one, watch flagsChanged and keyDown with NSEvent global and local monitors, which need Accessibility access, and act on a clean tap, pressed and released with no other key or modifier in between, so the key still types characters.",
  clipboard: "A feature that only copies text to the clipboard writes with clearContents() and then setString(_:forType:) and never reads or polls NSPasteboard.general, so it triggers no paste-access prompt. Only a feature the PRD has read the clipboard watches it: poll NSPasteboard.general changeCount and read its contents only after changeCount changes. The first programmatic read can show the system paste-access prompt; on macOS 15.4 and later (check with #available), read NSPasteboard.general.accessBehavior and, when it is alwaysDeny, stop reading and show the user how to allow access in System Settings. Snapshot and restore the user's clipboard only when the PRD promises to preserve it.",
  "background-startup": "Register and unregister SMAppService.mainApp from an explicit toggle. After register(), read status: requiresApproval means the user must approve the app in System Settings > Login Items, so say so and offer SMAppService.openSystemSettingsLoginItems(); show the toggle as on only when status is enabled. Denial leaves manual launch available.",
...
const swiftDesktop: PresetContract = {
  ...
  packagingRules: ["Scripts/package_app.sh is the sole packaging authority for release build, app assembly, resources, signing, strict verification, and DMG creation.", "Assemble one native arm64 macOS .app from the Swift Package Manager release executable, Info.plist, App.entitlements, and AppIcon.icns.", "Ad-hoc sign local builds or use an explicitly supplied Developer ID for distribution, then require codesign --verify --deep --strict.", "Register and launch the installed bundle through LaunchServices.", "An ad-hoc rebuild is a new app to macOS privacy controls: after reinstalling, expect its privacy and Keychain prompts again and re-grant any Accessibility access to the new build in System Settings; this is expected, not a defect."],
