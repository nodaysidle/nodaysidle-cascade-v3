...
function renderARD(graph: ProjectGraph): string {
  ...
  return [
    ...
    section("State Ownership", bullets([
      ...
      ...(graph.persistence.appFilesPlacement ? [`App file placement: ${graph.persistence.appFilesPlacement}`] : []),
      ...graph.owners.map(owner => `${owner.id} owns state mutated through ${owner.implementationFile}.`),
    ])),
    // Every preset lifecycle rule and every lifecycle requirement is already a lifecycle contract.
    section("Platform Lifecycle", bullets(lifecycle)),
    section("Persistence Placement", bullets([graph.persistence.decision, `Settings: ${graph.persistence.settingsPlacement}`, `Records: ${graph.persistence.recordsPlacement}`, ...(graph.persistence.appFilesPlacement ? [`App files: ${graph.persistence.appFilesPlacement}`] : [])])),
...
function renderAGENTS(graph: ProjectGraph): string {
  ...
  return [
    ...
    section("Execution Rules", bullets([
      ...
      "Keep test doubles (in-memory stores, console-only output, fake platform calls) in test files only; production entry points call the real owners named in the contracts.",
