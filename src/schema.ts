...
export function buildBlueprintInstructions(input: BlueprintInstructionInput): string {
  return [
    ...
    "Declare a document dataObject for every file or folder the user chooses to open, import, or save, such as an imported source file or an export, and list it in the usesData of every feature that reads or writes it. File access is granted only to features that list a document dataObject. Set its writeMode to atomic-replace when a failed write must leave an existing file at that location unchanged.",
