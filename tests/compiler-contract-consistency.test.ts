...
const blueprint: SemanticBlueprint = {
  ...
  dataObjects: [
    { name: "Note File", purpose: "Plain-text content of one note file.", sensitivity: "personal", retentionIntent: "Keep the file on the local filesystem until the user deletes it." },
    { name: "Editor Buffer", purpose: "In-memory text and cursor state for the editing session.", sensitivity: "personal", retentionIntent: "Discarded when the window closes and not written to disk." },
    { name: "Typography Preferences", purpose: "Font family and font size for the editor.", sensitivity: "internal", retentionIntent: "Keep in UserDefaults until the user resets them." },
    { name: "Search Index", purpose: "Derived searchable terms from note file names.", sensitivity: "personal", retentionIntent: "Held in memory for the session and not written to disk." },
  ],
  externalServices: [],
...
describe("compiler contract consistency", () => {
  ...
  it("keeps one atomic save path, session-only workspace state, and trigger-only shortcuts out of data links", async () => {
    const editor: SemanticBlueprint = {
      ...
      dataObjects: [
        { name: "Note File", purpose: "Plain-text content of one note file.", sensitivity: "personal", retentionIntent: "Kept on the local filesystem until the user deletes it." },
        { name: "Keybinding Preferences", purpose: "Keybinding assignments for editor commands.", sensitivity: "internal", retentionIntent: "Retained in UserDefaults until the user resets them." },
        { name: "Workspace Folder Reference", purpose: "Identifies the local folder whose notes are searched.", sensitivity: "internal", retentionIntent: "Retained only for the current session or until the user selects a different folder." },
        { name: "Temporary Write File", purpose: "Holds note contents during a write before being renamed over the destination file.", sensitivity: "personal", retentionIntent: "Removed immediately after a successful rename or cleaned up after a failed write." },
      ],
      externalServices: [],
      platformNeeds: ["filesystem", "local-storage"],
      qualityRequirements: ["Typing stays responsive"],
  ...
  it("keeps temp-file saves, session folders, shortcut triggers, and unstated defaults consistent", async () => {
    const editor: SemanticBlueprint = {
      ...
      dataObjects: [
        { name: "Note File", purpose: "Plain-text content of one note file.", sensitivity: "personal", retentionIntent: "Kept on the local filesystem until the user deletes it." },
        { name: "Typography Preferences", purpose: "Font family and font size for the editor.", sensitivity: "internal", retentionIntent: "Kept in UserDefaults until the user resets them." },
        { name: "Workspace Folder Reference", purpose: "Identifies the local folder available to search.", sensitivity: "internal", retentionIntent: "Retained only for the current session or until the user selects a different folder." },
        { name: "Temporary Write File", purpose: "Holds note contents during a write before being renamed over the destination file.", sensitivity: "personal", retentionIntent: "Removed immediately after a successful rename or cleaned up after a failed write." },
      ],
      externalServices: [],
      platformNeeds: ["filesystem", "local-storage"],
  ...
  it("classifies a named rename scratch file, keeps a search index off save features, and writes a grammatical problem statement", async () => {
    const editor: SemanticBlueprint = {
      ...
      dataObjects: [
        { name: "Note File", purpose: "Stores the plain-text contents of one note as a .txt file.", sensitivity: "personal", retentionIntent: "Retained until the user deletes or moves the file." },
        { name: "Temporary Save File", purpose: "Holds note contents during a background write before being renamed over the destination file.", sensitivity: "personal", retentionIntent: "Removed immediately after a successful rename or on the next save attempt after a failure." },
        { name: "Workspace Search Index", purpose: "Holds in-memory searchable representations of note file names and contents for fuzzy search.", sensitivity: "personal", retentionIntent: "Held only in memory for the duration of the app session and discarded on quit." },
      ],
      externalServices: [],
      platformNeeds: ["filesystem", "local-storage"],
      qualityRequirements: ["Typing stays responsive"],
