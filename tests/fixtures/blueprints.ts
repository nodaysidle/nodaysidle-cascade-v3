import type { FeaturePlatformNeed, FeatureRecovery, FeatureSurface, PlatformNeed, SemanticBlueprint } from "../../src/schema"
...
interface FeatureUses {
  readonly platform?: readonly FeaturePlatformNeed[]
...
function blueprint(seed: FixtureSeed): SemanticBlueprint {
  return {
    ...
    // Pipeline tests use one-sentence ideas, so sentence 1 covers every feature.
    ideaCoverage: [{ sentence: 1, features: seed.features.map(item => item.name), outsideFeatures: "none" }],
  }
}
